...
<html lang="en">
  <head>
    ...
    <title>Collage Space|||Manage Faculty</title>
    <!-- plugins:css -->
...
$sql="SELECT tbladmin.StuID,tbladmin.ID as sid,tbladmin.StudentName,tbladmin.StudentEmail,tbladmin.DateofJoining,tblclass.ClassName,tblclass.Section from tbladmin join tblclass on tblclass.ID=tbladmin.StudentClass LIMIT $offset, $no_of_records_per_page";
$query = $dbh -> prepare($sql);
$query->execute();
...
{               ?>   
                          <tr>
                            ...
                            <td>
                              <div><a href="edit-admin-detail.php?editid=<?php echo htmlentities ($row->sid);?>"><i class="icon-eye"></i></a>
                                                || <a href="manage-admin.php?delid=<?php echo ($row->sid);?>" onclick="return confirm('Do you really want to Delete ?');"> <i class="icon-trash"></i></a></div>
                            </td> 
                          </tr><?php $cnt=$cnt+1;}} ?>
                        </tbody>
                      </table>
